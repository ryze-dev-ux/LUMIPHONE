...
// Carrega catálogo salvo ou usa o padrão; persiste em qualquer mutação
const _savedProducts = localStorage.getItem('lumiProducts');
const products = _savedProducts ? JSON.parse(_savedProducts) : [...DEFAULT_PRODUCTS];
function saveProducts() { localStorage.setItem('lumiProducts', JSON.stringify(products)); }
...
function _setDetailImage(product, colorName) {
  ...
  // Thumbs
  document.querySelectorAll('.gallery-thumb-inner').forEach(t => {
    let i = t.querySelector('img');
    if (!i) { i = document.createElement('img'); i.style.cssText = 'width:100%;height:100%;object-fit:cover;border-radius:6px'; t.appendChild(i); }
    i.src = cd.img;
  });
}
...
function renderCartItems() {
  ...
  el.innerHTML = cart.map((item, idx) => {
    ...
    return `
    <div class="cart-item">
      <div class="cart-item-img"><div class="cart-item-img-inner"></div></div>
